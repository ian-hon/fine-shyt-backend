...
// use crate::{
//     AppState,
//     hermes_error::{self, HermesError, HermesFormat},
//     ws_statemachine::SocketContainer,
// };
...
//     return urlencoding::decode(q.get(&k.to_string()).unwrap().clone().as_str())
//         .unwrap()
//         .to_string();
// }
...
// ) -> impl IntoResponse
// where
//     F: Fn(AppState, Session, HashMap<String, String>, SocketAddr, WebSocketUpgrade) -> Fut,
...
//         }
//     }
//     let session_id = RawSessionID {
//         id: query.get("session_id").unwrap().to_string(),
//     };

//     match session_id.into_session(&app_state.db).await {
//         Ok(s) => match hermes_error::check(&query, hermes_check) {
...
// ) -> impl IntoResponse
// where
//     F: Fn(Pool<Sqlite>, Session, HashMap<String, String>) -> Fut,
